...
# ItemModel 上走 Appoption 的字段：通用 seg + 服务/商品/卡类专用 seg（srv*/goods*/cardtype*）
_ITEM_APPOPTION_SEG_BASES = (
...
SEGS = SEGS_CORE + [
    (prefix + base, '%s-%s' % (ptitle, blabel))
    for prefix, ptitle in (('srv', '服务项目'), ('goods', '商品'), ('cardtype', '卡类'))
    for base, blabel in _ITEM_APPOPTION_SEG_BASES
]

ITEM_APPOPTION_FIELD_NAMES = tuple(b for b, _ in _ITEM_APPOPTION_SEG_BASES)
...
BRAND = Appoption.objects.filter(flag='Y', seg='brand').values_list('itemname', 'itemvalues')
VIPLEVEL =  Appoption.objects.filter(flag='Y', seg='viplevel').values_list('itemname', 'itemvalues')
DISCOUNTCLASS= Appoption.objects.filter(flag='Y', seg='discountclass').values_list('itemname', 'itemvalues')

# DISPLAYCLASS1= Appoption.objects.filter(flag='Y', seg='displayclass1').values_list('itemname', 'itemvalues')
# DISPLAYCLASS2= Appoption.objects.filter(flag='Y', company=COMPANY, seg='displayclass2').values_list('itemname', 'itemvalues')
...
MARKETCLASS1= Appoption.objects.filter(flag='Y', seg='marketclass1').values_list('itemname', 'itemvalues')
MARKETCLASS2= Appoption.objects.filter(flag='Y', seg='marketclass2').values_list('itemname', 'itemvalues')
MARKETCLASS3= Appoption.objects.filter(flag='Y', seg='marketclass3').values_list('itemname', 'itemvalues')
MARKETCLASS4= Appoption.objects.filter(flag='Y', seg='marketclass4').values_list('itemname', 'itemvalues')
FINANCECLASS1= Appoption.objects.filter(flag='Y', seg='financeclass1').values_list('itemname', 'itemvalues')
FINANCECLASS2= Appoption.objects.filter(flag='Y', seg='financeclass2').values_list('itemname', 'itemvalues')
ARCHIVEMENTCLASS1 = Appoption.objects.filter(flag='Y', seg='archivementclass1').values_list('itemname', 'itemvalues')
ARCHIVEMENTCLASS2 = Appoption.objects.filter(flag='Y', seg='archivementclass2').values_list('itemname', 'itemvalues')
BODYPARTS1 = Appoption.objects.filter(flag='Y', seg='bodyparts1').values_list('itemname', 'itemvalues')
BODYPARTS2 = Appoption.objects.filter(flag='Y', seg='bodyparts2').values_list('itemname', 'itemvalues')
TAGS = Appoption.objects.filter(flag='Y', seg='tags').values_list('itemname', 'itemvalues')
VIPTAGS = Appoption.objects.filter(flag='Y', seg='viptags').values_list('itemname', 'itemvalues')
SOURCE= Appoption.objects.filter(flag='Y', seg='source').values_list('itemname', 'itemvalues')
...
class ItemModel(models.Model):
    ...
    brand = models.CharField(max_length=16, blank=True, null=True, verbose_name='品牌')
    discountclass = models.CharField(max_length=16, choices=DISCOUNTCLASS, default='', blank=True, null=True, verbose_name='折扣分类')
    displayclass1 = models.CharField(max_length=16, default='', blank=True, null=True, verbose_name='显示分类（方法一）')
    displayclass2 = models.CharField(max_length=16, default='', blank=True, null=True, verbose_name='显示分类（方法二）')
    marketclass1 = models.CharField(max_length=16,  default='', blank=True, null=True, verbose_name='项目营销分类（方法一）')
    marketclass2 = models.CharField(max_length=16,  default='', blank=True, null=True, verbose_name='项目营销分类（方法二）')
    marketclass3 = models.CharField(max_length=16,  default='', blank=True, null=True, verbose_name='项目营销分类（方法三）')
    marketclass4 = models.CharField(max_length=128, default='', blank=True, null=True, verbose_name='项目营销分类（方法四）')
    financeclass1 = models.CharField(max_length=16, default='', blank=True, null=True, verbose_name='项目财务分类1')
    financeclass2 = models.CharField(max_length=16, default='', blank=True, null=True, verbose_name='项目财务分类2')
    archivementclass1 = models.CharField(max_length=16, default='', blank=True, null=True, verbose_name='业绩分类1')
    archivementclass2 = models.CharField(max_length=16, default='', blank=True, null=True, verbose_name='业绩分类2')
    ...
    tags = MultiSelectField(choices=TAGS, max_length=128, default='', blank=True, null=True, verbose_name='标签')
    mnemoniccode=models.CharField(max_length=16, blank=True, null=True, verbose_name='助记码')
    storelist = MultiSelectField(choices=STOREINFO, max_length=128, blank=True, null=True, verbose_name='可用门店')
    ...
    @staticmethod
    def specialized_appoption_seg(model_cls_name: str, base_seg: str) -> str:
        """serviece→srv、goods→goods、cardtype→cardtype 前缀 + 通用 seg（如 marketclass1→srvmarketclass1）。"""
        mn = (model_cls_name or "").lower()
        prefix = {"serviece": "srv", "goods": "goods", "cardtype": "cardtype"}.get(mn)
        if not prefix:
            return base_seg
        return prefix + base_seg

    def _appoption_qs(self, seg: str):
        return Appoption.objects.filter(company=self.company, flag="Y", seg=seg)

    def apply_appoption_field_choices(self, field_name: str, base_seg: str):
        """单个字段：优先专用 seg，无数据则回退通用 seg；brand 再回退全库 flag=Y。"""
        spec = ItemModel.specialized_appoption_seg(self.__class__.__name__, base_seg)
        qs = self._appoption_qs(spec)
        if spec != base_seg and not qs.exists():
            qs = self._appoption_qs(base_seg)
        if base_seg == "brand" and not qs.exists():
            qs = Appoption.objects.filter(flag="Y", seg="brand")
        self._meta.get_field(field_name).choices = qs.values_list("itemname", "itemvalues")

    def apply_itemmodel_appoption_choices(self):
        """ItemModel 中与 Appoption 绑定的分类字段（含 MultiSelect 的 bodyparts1/tags）。"""
        for base_seg in ITEM_APPOPTION_FIELD_NAMES:
            self.apply_appoption_field_choices(base_seg, base_seg)

    def apply_displayclass_choices(self):
        """兼容旧调用：与 apply_itemmodel_appoption_choices subset 等价。"""
        self.apply_appoption_field_choices("displayclass1", "displayclass1")
        self.apply_appoption_field_choices("displayclass2", "displayclass2")

    def get_brand_list(self):
        brandlist = Appoption.objects.filter(company=self.company,  flag='Y',  seg='brand').values_list(
            'itemname',  'itemvalues')
        # print('get_brand_list brand list:',  brandlist)
        return brandlist

    def __init__(self,  *args,  **kwargs):
        super(ItemModel,  self).__init__(*args,  **kwargs)
        self.apply_itemmodel_appoption_choices()
        ...
        # print('brandlist', self._meta.get_field('brand').choices )
        # print('baseinfo.models itemmode company', self.company, self._meta.get_field('brand').choices)

    def get_brand_choices(self):
        rcs =  Appoption.objects.filter(company=self.company,  flag='Y',  seg='brand').values_list( 'itemname',  'itemvalues')
...
class Paymode(GenesisModel):
    ...
    visibleflag = models.CharField(max_length=8, choices=FLAG, default='Y', blank=True, null=True, verbose_name='是否显示')
...
class Hdsysuser(GenesisModel):
    ...
    sys_passwd = models.CharField(
        db_column='SYS_PASSWD',
        max_length=32,
        verbose_name='密码',
        help_text='已废弃：仅迁移/同步 Django User 时使用，登录不再校验此字段',
    )
    ...
    report_fromdate = models.CharField(max_length=16, blank=True, null=True, verbose_name='报表开始日期')
    ...
    def is_sys_admin(self):
        return (self.sys_adm or '').strip().upper() == 'Y'

    def user_uuid_key(self):
        """useright.sys_userid 存 32 位无连字符 uuid。"""
        return str(self.uuid).replace('-', '')

    def parse_storelist_codes(self):
        """解析 storelist（MultiSelect 或逗号串）+ 默认 storecode。"""
        codes = []
        raw = self.storelist
        if raw:
            if isinstance(raw, (list, tuple)):
                ...
                for part in str(raw).replace('，', ',').split(','):
                    ...
                    if p:
                        codes.append(p)
        home = (self.storecode or '').strip()
        if home and home not in codes:
            codes.insert(0, home)
        return codes

    def allowed_storecodes(self):
        """可登录/操作的门店编号列表；管理员=公司下全部有效门店。"""
        if self.is_sys_admin():
            return list(
                Storeinfo.objects.filter(company=self.company, flag='Y')
                .order_by('storecode')
                .values_list('storecode', flat=True)
            )
        return self.parse_storelist_codes()
    ...
    def can_access_store(self, storecode):
        ...
        if self.is_sys_admin():
            return Storeinfo.objects.filter(
                company=self.company, flag='Y', storecode=sc
            ).exists()
        return sc in self.allowed_storecodes()

    def accessible_stores(self):
        """带名称的门店列表，供登录后选店。"""
        codes = self.allowed_storecodes()
        if not codes:
            return []
        qs = Storeinfo.objects.filter(
            company=self.company, flag='Y', storecode__in=codes
        ).order_by('storecode')
        return [
            {'storecode': s.storecode, 'storename': (s.storename or s.storecode or '').strip()}
            for s in qs
        ]

    def module_rights(self, storecode=None):
        """当前用户在指定门店的功能权限（useright 表）。"""
        sc = (storecode or self.storecode or '').strip()
        if not sc:
            return Useright.objects.none()
        return Useright.objects.filter(
            company=self.company,
            storecode=sc,
            sys_userid=self.user_uuid_key(),
            flag='Y',
        ).order_by('sys_module')
...
PCODELIST= Paymode.objects.filter(company=COMPANY, flag='Y').values_list('pcode', 'pname')
class Cardsupertype(GenesisModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._meta.get_field('normal_pcode').choices =  Paymode.objects.filter(company=self.company, flag='Y').values_list('pcode', 'pname')
        self._meta.get_field('present_pcode').choices =  Paymode.objects.filter(company=self.company, flag='Y').values_list('pcode', 'pname')

    def get_pcodelist(self):
        PCODELIST = Paymode.objects.filter(company=self.company,  flag='Y').values_list('pcode',  'pname')
        return PCODELIST
...
CARDSUPTYPE = Cardsupertype.objects.filter(flag='Y').values_list('code', 'name')
class Cardtype(GenesisModel, ItemModel, ArchivementBaseModel):
    ...
    def __init__(self,  *args,  **kwargs):
        super(ItemModel,  self).__init__(*args,  **kwargs)
        # CARDSUPTYPE = Cardsupertype.objects.filter(flag='Y').values_list('code',  'name')
        self._meta.get_field('suptype').choices = Cardsupertype.objects.filter( flag='Y', company=self.company).values_list( 'code',  'name')
        self.apply_itemmodel_appoption_choices()
...
class CardtypeVsDiscountClass(GenesisModel):
    cardtypeuuid = models.ForeignKey(Cardtype, db_column='cardtypeuuid', blank=True, null=True, on_delete=models.DO_NOTHING,verbose_name='卡类')
...
#    vipflag = models.CharField(max_length=1,  blank=True,  null=True)
    ...
    mtcode = models.CharField(max_length=20,  blank=True,  null=True, verbose_name='手机号')
    ...
    status = models.CharField(max_length=8,  choices=VIPSTATUS, blank=True,  null=True, verbose_name='当前状态')
    ...
    def __init__(self, *args, **kwargs):
        super(Vip,  self).__init__(*args,  **kwargs)
        self._meta.get_field('storecode').choices = Storeinfo.objects.filter(flag='Y',company=self.company).values_list('storecode', 'storename')
        self._meta.get_field('viplevel').choices = Appoption.objects.filter(flag='Y',  company=self.company, seg='viplevel').values_list('itemname',  'itemvalues')
...
class VipAddress(GenesisModel):
    vipuuid=models.ForeignKey(Vip, db_column='vipuuid', blank=True, null=True, on_delete=models.DO_NOTHING,verbose_name='会员')
...
POSITION = Position.objects.filter(company=COMPANY,  flag='Y').values_list('positioncode',  'positiondesc')
STORECODE = Storeinfo.objects.filter(company=COMPANY, flag='Y').values_list('storecode',  'storename')
TEAMLIST = Team.objects.filter(company=COMPANY,  flag='Y').values_list('teamid',  'teamname')
...
class Empl(BaseModel):
    ...
    position = models.CharField(db_column='POSITION',  max_length=20,  blank=True,  null=True, verbose_name='职位')  # Field name made lowercase.
 ...
 #    teamid = models.ForeignKey(Team, db_column='teamid', max_length=16,  blank=True,  null=True, on_delete=models.SET_NULL, verbose_name='业务组别')
    ...
    storecode = models.CharField(max_length=16, blank=True, null=True, verbose_name='所属门店')
    ...
    openid = models.CharField(max_length=32, blank=True, null=True, verbose_name='微信openid')
    unionid = models.CharField(max_length=32, blank=True, null=True, verbose_name='微信unionid')
    storelist = MultiSelectField(blank=True, null=True, verbose_name='可用门店')
...
class Goods(GenesisModel, ItemModel, ArchivementBaseModel):
    ...
    GOODSCTS = Goodsct.objects.filter(flag='Y').values_list('goodsct', 'goodsctname')
    ...
    def __init__(self,  *args,  **kwargs):
        ...
        self._meta.get_field('goodsct').choices = Goodsct.objects.filter(flag='Y').values_list('goodsct', 'goodsctname')

    def save(self,  force_insert=False,  force_update=False,  using=None, 
             update_fields=None):
        self.last_modified = datetime.now()
        super().save()
        if self.company in ['dieshang', 'dsdemo'] :
            print('model gcode info:', self.gcode,  self.gcode[0])
            if self.gcode[0] in ['2',  '3']:
                self.saleflag='N'
                self.save()
    ...
    def set_qtybyspec(self):
        try:
            if len(self.spec) >0 and self.spec != None:
                qty = float(re.findall(r'\d+', self.spec)[0])
                if self.qty != qty:
                    print('goods spec to qty:',  self.gcode,  self.spec, self.qty,  qty)
                    self.qty=qty
                    self.save()
        except Exception as e:
            print('goods spec to qty error', self.gcode,  self.spec,  e)

class Goodsprice(GenesisModel):
    goodsuuid = models.ForeignKey('Goods',  db_column='goodsuuid',  null=True,  on_delete=models.DO_NOTHING,  verbose_name='商品')
    gcode = models.CharField(db_column='gcode',  max_length=40, blank=True,  verbose_name='商品编码')  # Field name made lowercase.
...
class Serviece(GenesisModel, ItemModel, ArchivementBaseModel):
    ...
    def save(self,  force_insert=False,  force_update=False,  using=None, 
        ...
        cardtype = Cardtype.objects.get_or_create(company=self.company, ttype='S', cardtype=self.svrcdoe)[0]
        ...
        # servieceprice.save()
        pricelist = Servieceprice.objects.filter(flag='Y', srvuuid=self.uuid)
        for item in pricelist:
            item.company = self.company
            item.srvcode = self.svrcdoe
            item.save()

        goodslist = Serviecegoods.objects.filter(flag='Y', srvuuid=self.uuid)
...
class Servieceprice(GenesisModel):
    ...
    #     print('servieceprice', self.company,  self.srvcode,  self.srvuuid)
    #     srv = Serviece.objects.get(flag='Y', company=self.company, svrcdoe=self.srvcode)
    #     self.company=srv.company
    #     # self.srvcode=srv.svrcdoe
    #     # self.save()
    #     super().save()
...
class Serviecegoods(GenesisModel):
    ...
    def __str__(self):
        return self.srvuuid.svrname

    def save(self,  force_insert=False,  force_update=False,  using=None, 
             update_fields=None):
        self.srvcode = self.srvuuid.svrcdoe
        self.gcode = self.goodsuuid.gcode
        super().save()

    @property
    def goodscost(self):
        return self.qty / self.goodsuuid.qty * self.goodsuuid.buyprc
...
class Wharehouse(GenesisModel):
    ...
    saleatr  = MultiSelectField(choices=SALEATR, max_length=128, null=True, blank=True, verbose_name='允许操作')
    tiwhcodelist =  MultiSelectField(choices=WHCODELIST, max_length=128, blank=True, null=True, verbose_name='可以转出的仓库')
    ...
    def __init__(self, *args, **kwargs):
        super(Wharehouse,  self).__init__(*args,  **kwargs)
        self._meta.get_field('tiwhcodelist').choices = Wharehouse.objects.filter(company=self.company, flag='Y').values_list('wharehousecode', 'wharehousename')
        self._meta.get_field('storecode').choices = Storeinfo.objects.filter(flag='Y', company=self.company).values_list('storecode',  'storename')
        # super(Cardsupertype,  self).__init__(*args,  **kwargs)
        # self._meta.get_field('pcode').choices =  Paymode.objects.filter(company=self.company, flag='Y').values_list('pcode', 'pname')
...
# WHCODELIST=Wharehouse.objects.filter(company=COMPANY, flag='Y').values_list('wharehousecode', 'wharehousename')
