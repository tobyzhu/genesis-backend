...
from .models import Appoption,Empl,Position,Vip,Serviece,Servieceprice,Srvtopty,Goods,Goodsct,Cardvsdi,Cardtype,Promotions,Hdsysuser,Useright,Storeinfo
# import baseinfo.models
import common.constants
from common.constants import COMPANYLIST_WITHOUT_MTCODE
...
def get_viplist_byecode(request):
    ...
    #
    companylist1 = ['yfy','yiren']
    companylist2 = ['salon','demo']
    companylist3 = ['yiren']
    COMPANYLIST_BOTHECODE = ['yiren']
    # url= request.GET['url']
    if request.method == 'GET':
        if searchvalueflag=='N':
            sql = " select distinct a.uuid uuid, a.viptype viptype, vcode ,vname, mtcode,pinyin, b.ecode ecode, b.ename ename, a.viplevel,a.birth" \
                  " from vip a, empl b" \
                  " where a.company = b.company and a.flag='Y' and a.valiflag='Y' and a.storecode=b.storecode and ( a.ecode = b.ecode ) " \
            ...
            if company in companylist1:
                try:
                    ...
                    if empl.position in ('100','110') and empl.company=='yiren':
                        sql = " select distinct a.uuid uuid, a.viptype viptype, vcode ,vname,'' mtcode, pinyin, b.ecode ecode, b.ename ename, a.viplevel,a.birth" \
                              " from vip a, empl b" \
                              " where a.company = b.company and a.flag='Y'  and a.valiflag='Y' and a.storecode=b.storecode and ( a.ecode2 = b.ecode )  " \
                              " and a.company =%s  and a.storecode =%s and b.ecode= %s" \
                              " order by pinyin "
                        params = (company + ' ' + storecode + '  ' + ecode).split()
                    else:
                        sql = " select distinct a.uuid uuid, a.viptype viptype, vcode ,vname,'' mtcode, pinyin, b.ecode ecode, b.ename ename, a.viplevel,a.birth" \
                              " from vip a, empl b" \
                              " where a.company = b.company and a.flag='Y' and a.valiflag='Y' and a.storecode=b.storecode and ( a.ecode = b.ecode or a.ecode2=b.ecode )  " \
                              " and a.company =%s  and a.storecode =%s  and b.ecode= %s " \
                              " order by pinyin "

                        params = (company + ' ' + storecode + ' ' + ecode).split()
                except Exception as e:
                    print('getvipbyecode error',e)
                    sql = " select distinct a.uuid uuid, a.viptype viptype, vcode ,vname,'' mtcode, pinyin, b.ecode ecode, b.ename ename, a.viplevel,a.birth" \
                          " from vip a, empl b" \
                          " where a.company = b.company and a.flag='Y'  and a.valiflag='Y' and a.storecode=b.storecode and ( a.ecode = b.ecode or a.ecode2=b.ecode )  " \
                          " and a.company =%s  and a.storecode =%s and b.ecode= %s" \
        ...
        if searchvalueflag=='Y':
            ...
            sql = " select distinct a.uuid uuid, a.viptype viptype, vcode ,vname,mtcode, pinyin, b.ecode ecode, b.ename ename, a.viplevel,a.birth" \
                  " from vip a, empl b" \
                  " where a.company = b.company and a.flag='Y'  and a.valiflag='Y' and a.storecode=b.storecode   " \
            ...
            if company in COMPANYLIST_WITHOUT_MTCODE:
                sql = " select distinct a.uuid uuid, a.viptype viptype, vcode ,vname,'' mtcode, pinyin, b.ecode ecode, b.ename ename, a.viplevel,a.birth" \
                      " from vip a, empl b" \
                      " where a.company = b.company and a.flag='Y'  and a.valiflag='Y' and a.storecode=b.storecode   " \
                      " and (a.ecode = b.ecode )" \
...
def get_vipbaseinfo(request):
    ...
    if request.method == 'GET':
        if company in COMPANYLIST_WITHOUT_MTCODE:
            # print('no mtcode')
            sql = ' select uuid , storecode, vcode, vname, viptype, viplevel,ecode,getbaseinfo(company,storecode,"empl",ecode,"ename") emame, ecode2, getbaseinfo(company,storecode,"empl",ecode2,"ename") emame2 ' \
                  ' from vip ' \
                  ' where 1=1 and company = %s and storecode=%s and (uuid = %s  or vcode = %s) '
        else:
            sql = ' select uuid ,storecode, vcode, vname, viptype, viplevel,mtcode,ecode,getbaseinfo(company,storecode,"empl",ecode,"ename") emame, ecode2,getbaseinfo(company,storecode,"empl",ecode2,"ename") emame2 ' \
                  ' from vip ' \
                  ' where 1=1 and company = %s and storecode=%s and (uuid = %s  or vcode = %s) '
...
def get_nextvcode(company,storecode):
    ...
    prefix = storecode
    codelength=5
    total_length = len(prefix)+codelength
    vip = Vip.objects.filter(flag='Y',company=company,storecode=storecode, vcode__startswith=storecode).extra(where=['length(vcode) = 7']) .order_by('-vcode').first()
    print(vip)
    if len(vip.vcode)>0 :
        print(vip.vcode,'len(storecode)=', len(storecode), 'len(vip.vcode) - len(storecode)=',len(vip.vcode) - len(storecode) )
        nextcode = vip.vcode[len(storecode):len(vip.vcode)]
...
# 根据商品规格描述 解析出商品容量
def setgoodsqty_byspec(request):
    try:
        company = request.GET['company']
    except:
        company = common.constants.COMPANYID

    goods = Goods.objects.filter(flag='Y',company=company, valiflag='Y')
    for good in goods:
        good.set_qtybyspec()
...
def get_indexImageUrl(request):
    """
    小程序首页轮播图：返回图片 URL 的 JSON 数组。
    优先使用 mall.Banner（apppage 为 index / home / 请求参数 page）；无数据时返回默认占位图 URL。
    """
    from django.db.models import Q

    company = request.GET.get("company") or common.constants.COMPANYID
    page = request.GET.get("page") or "index"
    urls = []
    try:
        from mall.models import Banner

        apppage_q = Q(apppage=page) | Q(apppage="index") | Q(apppage="home")
        qs = (
            Banner.objects.filter(flag="Y", company=company)
            .filter(apppage_q)
            .order_by("orderno", "id")
        )
        for b in qs:
            if b.bannerimage:
                ...
                if u.startswith("http://") or u.startswith("https://"):
                    ...
                    urls.append(request.build_absolute_uri(u))
    except Exception:
        pass
    if not urls:
        urls = [
...
def get_salonlist(request):
    """
    小程序「沙龙/门店」下拉：返回 uuid + salon_name（与 baseinfo 门店表 Storeinfo 对应）。
    """
    company = request.GET.get("company") or common.constants.COMPANYID
    stores = (
        Storeinfo.objects.filter(company=company, flag="Y")
        .order_by("storecode", "uuid")
    )
    data = [
        {
            "uuid": str(s.uuid),
            "salon_name": (s.storename or s.storecode or "").strip() or str(s.uuid),
