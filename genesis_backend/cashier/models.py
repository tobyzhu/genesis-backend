...
# 与 common.models.Sequence 共用同一张表，避免 E028 重复 db_table
from common.models import Sequence  # noqa: F401
...
class Expense(GenesisModel):
    ...
    @property
    def cardsuptype(self):
        if self.ttype in ('C','I'):
            try:
                cardinfo = Cardinfo.objects.get(company=self.company,ccode=self.srvcode)
                return cardinfo.cardtypeuuid.suptype
...
class Toll(GenesisModel):
    ...
    bankaccount =  models.CharField(max_length=32,db_column='bankaccount',choices=(),blank=True,null=True,verbose_name='银行账号')
    confirm_ecode = models.CharField(max_length=32,db_column='confirm_ecode', choices=(),blank=True,null=True,verbose_name='财务确认人')
    ...
    def __init__(self, *args, **kwargs):
        super(Toll, self).__init__(*args, **kwargs)
        self._meta.get_field('pcode').choices = Paymode.objects.filter(company=self.company, flag='Y').values_list( 'pcode', 'pname')
        self._meta.get_field('bankaccount').choices = BankAccount.objects.filter(company=self.company, flag='Y').values_list( 'accountcode', 'bankname')
        self._meta.get_field('confirm_ecode').choices = Empl.objects.filter(company=self.company, flag='Y').values_list( 'ecode', 'ename')
...
class TollCash(Toll):
    ...
    def get_queryset(self):
        pcodelist = Paymode.objects.filter(flag='Y',company=self.company,iscash='1').values_list('pcode')
        return Toll.objects.filter(flag='Y',pcode__in=pcodelist)

class ExpvstollCash(Expvstoll):
    ...
    #     leftmoney=0
    #     if len(self.ccode) >= 0:
    #         leftmoney=self.cardleftmoney
    #     return  leftmoney

class EmplArchivementDetail(GenesisModel):
    trans=models.ForeignKey(Expvstoll,db_column='transuuid',blank=True,null=True, on_delete=models.DO_NOTHING, verbose_name='交易')
...
class Cardhistory(GenesisModel):
    ...
    adjuestamount =  models.DecimalField(max_digits=10, decimal_places=2, default=0, blank=True, null=True,verbose_name='调整金额')
    ...
    adjuesttype = models.CharField(max_length=8, choices=CARDADJUESTTYPE,blank=True, null=True,verbose_name='金额调整类型')
    oldcardtonew = models.ForeignKey(Oldcardtonew,db_column='oldcardtonewuuid',on_delete=models.DO_NOTHING,blank=True,null=True)
...
class EmplDailyData(GenesisModel):
    empl = models.ForeignKey(Empl,db_column='empl',blank=True,null=True, on_delete=models.DO_NOTHING, verbose_name='员工')
...
class EarnestMoney(GenesisModel):
    ...
    hunguuid = models.ForeignKey(ExpvstollHung,db_column='hungsuuid',blank=True,null=True, on_delete=models.DO_NOTHING, verbose_name='开单记录')
    transuuid = models.ForeignKey(Expvstoll,db_column='transuuid',blank=True,null=True, on_delete=models.DO_NOTHING, verbose_name='交易记录')
    # exptxserno = models.CharField(max_length=32,blank=True,null=True,verbß
    # ose_name='交易流水号')
    # ditem = models.CharField(max_length=8,blank=True,null=True,verbose_name='项次')
    vipuuid = models.ForeignKey(Vip,db_column='vipuuid',blank=True,null=True, on_delete=models.DO_NOTHING, verbose_name='客人')
...
class VipItemInfo(GenesisModel):
    vip =  models.ForeignKey(baseinfo.models.Vip,db_column='vipuuid',blank=True,null=True, on_delete=models.DO_NOTHING ,verbose_name='客人')
...
class VipItemTransDetail(GenesisModel):
    sukid = models.CharField(max_length=32, blank=True, null=True, verbose_name='系统单号')
    ditem = models.CharField(max_length=8, blank=True, null=True, verbose_name='项次')
    vipitemtrans  =  models.ForeignKey(VipItemTransHead,db_column='vipitemtransuuid',blank=True,null=True, on_delete=models.DO_NOTHING, verbose_name='会员项目交易')
    vipiteminfo = models.ForeignKey(VipItemInfo,db_column='vipiteminfouuid',blank=True,null=True, on_delete=models.DO_NOTHING, verbose_name='会员项目')
    transtype = models.CharField(max_length=16, choices=VIPITEMTRANSTYPE,blank=True, null=True,verbose_name='项目交易类型')
    itemuuid = models.ForeignKey(baseinfo.models.Item,db_column='itemuuid', blank=True,null=True,on_delete=models.DO_NOTHING, verbose_name='项目')
    ttype = models.CharField(max_length=16,choices=TTYPE,blank=True,null=True,verbose_name='项目类型')
    itemcode = models.CharField(max_length=32, blank=True, null=True,verbose_name='项目编号')
