...
def init_baseinfo(request):
    ...
    # hdsysuser（保留 useright 桥接；登录密码请用 Django User）
    ...
    call_command('sync_genesis_profiles_from_hdsysuser', company=company, set_password=True)
...
def check_WifiList(request):
    ...
    if networktype=='wifi':
        ...
        print('bssid,ssid',bssid, ssid)
        try:
            wifi = WifiList.objects.get_or_create(SSID=ssid,BSSID=bssid)[0]
            print('wifi:',wifi.company,wifi.creater)
        except Exception as e:
            print('wifilist object error:',e, ssid, bssid )
...
def check_userpwd(request):
    ...
    from common.genesis_auth import authenticate_genesis_user, serialize_genesis_session

    auth = authenticate_genesis_user(company, usercode, password, storecode)
    if not auth:
        return HttpResponse(
            '500', content_type='application/x-www-form-urlencoded;charset=utf-8'
        )

    detail = request.GET.get('detail', '').lower() in ('1', 'true', 'json', 'yes')
    if detail:
        print('serialize_genesis_session(auth)', serialize_genesis_session(auth))
        return JsonResponse(serialize_genesis_session(auth))

    return HttpResponse(
        '200', content_type='application/x-www-form-urlencoded;charset=utf-8'
    )
