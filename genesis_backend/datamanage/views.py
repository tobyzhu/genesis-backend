...
from common.legacy_db import connect_youlan_sync
import uuid
...
def checkvip(request):
    company=request.GET['company']
    storecode=request.GET['storecode']

    vips = Vip.objects.filter(flag='Y',company=company, storecode=storecode).order_by('vcode')
    for vip in vips:
        vip.set_pinyin()
