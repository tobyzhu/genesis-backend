...
class Report(object):
    def __init__(self):
        self.company = 'JMJ'
        self.fromdate = '20201201'
        self.todate = '20210531'
        self.reportyear = '2021'
        self.beforeyear = '2020'
    ...
    def report1(self):
        ...
        for goodsrpt3 in goodsrpt3s:
            ...
            sql = " select a.storecode, g.rptcode1, getappoptionvalue(g.company,'goodsrpt1',g.rptcode1) rptname1, g.rptcode2,getappoptionvalue(g.company,'goodsrpt2',g.rptcode2) rptname2,"\
                  ...
                  "  and a.vsdate>= '20191201' and a.vsdate<= '20200701' "\
                  "  and g.rptcode1= '2020' "\
                  "  and g.rptcode3 = %s" \
                  "  and a.vipuuid in ( "\
                  ...
                  "          and a.vsdate>='20201201' "\
                  "          and a.vsdate<='20210531' "\
                  "          and g.rptcode1='2021' " \
                  "          and g.rptcode3 = %s" \
                  "      ) "\
            ...
            filename = 'c:/tmp/'+goodsrpt3['itemvalues']+'_20年购买，21年到店未购买的客户数量.xls'
    ...
    def report2(self):
        ...
        for goodsrpt3 in goodsrpt3s:
            ...
            sql = " select a.storecode, b.rptcode1,b.rptname1, b.rptcode2, b.rptname2, b.rptcode3,  b.rptname3,      count(distinct a.vipuuid) vipcnt    "\
                  ...
                  "     and b.srvcode=g.gcode     and a.vsdate>= '20191201' and a.vsdate<= '20200701'    "\
                  "      and g.rptcode1= '2020'  and g.rptcode2 = '10'    "\
                  "      ) a,    "\
                  "      (    "\
                  ...
                  "      where 1=1    and a.valiflag='Y'   and a.uuid = b.transuuid and b.srvcode=g.gcode    "\
                  "      and a.vsdate>= '20191201' and a.vsdate<= '20200701'    "\
                  "     and g.rptcode1= '2020'  and g.rptcode3 = %s    "\
                  "      ) b     "\
                  "  where a.vipuuid = b.vipuuid    "\
                  "  and a.vipuuid not in (    "\
                  "          select distinct vipuuid from expvstoll    "\
                  "          where 1=1            and valiflag = 'Y'    "\
                  "          and vsdate >= '20201201'           and vsdate <= '20210701'    "\
                  "          )    "\
            ...
            json_data = sql_to_json(sql, params)
            print(item,type(item),json_data)
            filename = 'c:/tmp/'+goodsrpt3['itemvalues']+'_20年即购买方位又购买属相，21年未到店客户数量.xls'
            json_to_excel(json_data,filename)
            # return json_data
            # return HttpResponse(json_data, content_type="application/json")
...
# 购买9宫方位的客户总数
# select a.storecode, count(distinct a.vipuuid) vipcnt
# from expvstoll a, expense b, goods c
# where 1=1
# and a.valiflag='Y'
# and a.EXPTXSERNO = b.EXPTXSERNO
# and b.srvcode=c.gcode
# and c.rptcode3 in ('101','102','103','104','105','106','107','108','109')
# and c.rptcode1='2021'
# and a.vsdate >='20201201'
# and a.vsdate <='20210531'
