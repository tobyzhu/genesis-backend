...
REPORTYEAR='2022'
RPTCODE1='2022'
YEARFROMDATE='20211222'
REPORTFROMDATE='20211130'
...
def ReadAndWrite(request):
        ...
        for value in readResult:
            ...
            getgcodemirrorsql = "select gcode2012 from gcodemirror where gcode ='" +value[7]+"'"
...
def SetPeriodData(request):
    ...
    goods = Goods.objects.filter(valiflag='Y').filter(rptcode1=rptcode1).order_by('gcode')
...
#     reportname = '关键阶段销售报表'
    ...
    for store in stores:
        ...
        for reportperiod in reportperiods:
            ...
            perioddataitem101 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='101')[0]
            perioddataitem102 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='102')[0]
            perioddataitem103 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='103')[0]
            perioddataitem104 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='104')[0]
            perioddataitem105 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='105')[0]
            perioddataitem106 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='106')[0]
            perioddataitem107 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='107')[0]
            perioddataitem108 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='108')[0]
            perioddataitem109 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='109')[0]
            perioddataitem112 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='112')[0]
            perioddataitem113 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='113')[0]
            perioddataitem117 = PeriodData.objects.get_or_create(reportperiod=reportperiod, company='JMJ', storecode=store.storecode,rptcode3='117')[0]
        ...
        for good in goods:
            ...
            for reportperiod in reportperiods:
                ...
                totalqty = Goodstranslog.objects.values('storecode', 'gcode', 'saleatr').annotate(sumqty=Sum('qty1')).filter(
                    ...
                    vdate__lte=todate).filter(saleatr='G')
                print(good.gcode,'totalqty=',totalqty,'yearfromdate=','todate=',todate)
        ...
        for whcode in whcodes:
            ...
            for period in periods:
                ...
                for salehead in saleheads:
                    ...
                    print('order qty:',orderqty, orderqty[item]['gcode'], 'order qty:', orderqty[item]['qty'], period.id)
                    for item in range(orderqty.count()):
                        ...
                        if Goods.objects.get(company=company,gcode=orderqty[item]['gcode']).rptcode1 == reportyear:
                            perioddata = PeriodData.objects.get_or_create(company=company, storecode=store.storecode,
                                                                reportperiod_id=period.id,
                                                                gcode=orderqty[item]['gcode'])[0]
