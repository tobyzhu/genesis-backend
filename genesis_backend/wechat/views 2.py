...
def get_appinfo(appcode):
    ...
    if appcode == "100":
        return {
            "appid": os.environ.get("WECHAT_APP_100_APPID", os.environ.get("WECHAT_APPID", "")),
            "secret": os.environ.get(
                "WECHAT_APP_100_SECRET", os.environ.get("WECHAT_APP_SECRET", "")
            ),
            "token": os.environ.get("WECHAT_APP_100_TOKEN", os.environ.get("WECHAT_TOKEN", "")),
            "my_username": os.environ.get(
                "WECHAT_APP_100_USERNAME", os.environ.get("WECHAT_USERNAME", "")
            ),
            "encodingAESKey": os.environ.get(
                "WECHAT_APP_100_AES_KEY", os.environ.get("WECHAT_ENCODING_AES_KEY", "")
            ),
            "mch_id": my_mch_id,
            "UFDODER_URL": os.environ.get(
                "WECHAT_UFDODER_URL", "https://api.mch.weixin.qq.com/pay/unifiedorder"
            ),
            "NOTIFY_URL": os.environ.get("WECHAT_PAY_NOTIFY_URL", NOTIFY_URL),
            "CREATE_IP": os.environ.get("WECHAT_SPBILL_CREATE_IP", ""),
            "nonce_str": "",
    ...
    if appcode == "200":
        return {
            "appid": os.environ.get("WECHAT_APP_200_APPID", ""),
            "secret": os.environ.get("WECHAT_APP_200_SECRET", ""),
            "token": os.environ.get("WECHAT_APP_200_TOKEN", os.environ.get("WECHAT_TOKEN", "")),
            "my_username": os.environ.get("WECHAT_APP_200_USERNAME", ""),
            "encodingAESKey": os.environ.get("WECHAT_APP_200_AES_KEY", ""),
        }
...
def WechatLogin(request):
    ...
    if not code:
        return JsonResponse({"code": 400, "message": "缺少code"}, status=400)
    ...
    if appcode=='200':
        ...
        resp_data = {
            ...
            "session_key":session_key,
            "openid":openid,
            "vipuuid": str(vip.uuid).replace('-','') if vip else '',
            'vip': None if vip is None else {
                'vipuuid':str(vip.uuid).replace('-',''),
                'vcode':vip.vcode,
                'vname':vip.vname,
                'mtcode':vip.mtcode,
                'ecode':vip.ecode,
...
def goodstoproduct(request):
    ...
    print('param',param,'access_token',access_token['access_token'])
    url= 'https://api.weixin.qq.com/product/spu/add?access_token={}'.format(access_token['access_token'])
    # url = 'https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token={}'.format(access_token['access_token'])
    # url = 'https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=ACCESS_TOKEN'
    # r = requests.post(url,json.dumps(param))
    print('url',url)
