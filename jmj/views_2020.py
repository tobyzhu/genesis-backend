...
def ReadAndWrite(request):
        ...
        read = pymysql.connect("qdxhz.imwork.net", "root", "shgv2014", "genesis")
        Rcursor = read.cursor()
        ...
        for value in readResult:
            ...
            getgcodemirrorsql = "select gcode2010 from gcodemirror where gcode ='" +value[7]+"'"
...
def InitPeriodData(request):
    ...
    for store in stores:
        goodslist=['20721']
        goods = Goods.objects.filter(company=company,rptcode1=rptcode1,gcode__in=goodslist)
        # print('goods',goods)
        for good in goods:
    ...
    for store in stores:
        ...
        for good in goods:
            ...
            for reportperiod in reportperiods:
                ...
                perioddataitem = PeriodData.objects.get(reportperiod=reportperiod,company=company,storecode=store.storecode,gcode=good.gcode,rptcode2=good.rptcode2,rptcode3=good.rptcode3)
